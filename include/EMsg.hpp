...
template <typename T, std::enable_if_t<std::is_base_of_v<::google::protobuf::Message, T>, std::nullptr_t> = nullptr, typename HeaderType = CMsgProtoBufHeader>
class EMsg {
    public:
        ...
        inline auto GetHeader() -> HeaderType {
            return m_header;
        }
    ...
    private:
        inline explicit EMsg(int eMsg, const T &protobuf, const CMsgProtoBufHeader &header) :
            m_eMsg(eMsg),
            m_jobName(), // Not sure if a response has a job name, but we ball
            m_protobuf(protobuf),
            m_header(header)
        {
